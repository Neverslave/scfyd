...
public class LoginController extends BaseJsonController {
    ...
    public @ResponseBody
    BaseRespBean userBind(HttpServletRequest request, HttpSession httpSession, @RequestBody UserLoginReqBean userLogin) throws IOException {

        Map<String, Object> paramMap = BeanUtil.beanToMap(userLogin);
        BaseRespBean respBean = this.userOperService.userBind(paramMap, (String) request.getParameter("openId"));
        return respBean;
    }
    ...
    private void setUserSession(UserLoginRespBean respBean, HttpSession httpSession) {
        ...
        httpSession.setAttribute(ScfConsts.SESSION_REPRESENT_ID, respBean.getRepresentId());
        httpSession.setAttribute(ScfConsts.SESSION_MENU_LIST, respBean.getMenuList());
        httpSession.setAttribute(ScfConsts.SESSION_REAL_NAME, respBean.getRealname());
    }
    ...
    @RequestMapping(value = "/login/phone", method = RequestMethod.POST, consumes = {"application/json"}, produces = {
            "application/json;charset=UTF-8"})
    public @ResponseBody
    BaseRespBean loginPhone(HttpServletRequest request, HttpSession httpSession,
                            @Valid @RequestBody LoginPhoneReqBean reqBean, BindingResult bindingResult) throws IOException {
        BaseRespBean respBean = new BaseRespBean();
        if (bindingResult.hasErrors()) {
            log.warn("bindingResult has error");
            respBean.setResult(ErrorCodeEnum.PARAM_VALID_ERROR);
        ...
        if (loginPhoneCodeExpireTime == null
                || (Calendar.getInstance().getTimeInMillis() - loginPhoneCodeExpireTime.getTime() > 60 * 1000)) {
            respBean.setResult(ErrorCodeEnum.LOGIN_CODE_EXPIRE);
            return respBean;
        }
    ...
    @RequestMapping(value = "/login/phoneCode", method = RequestMethod.POST, consumes = {
            "application/json"}, produces = {"application/json;charset=UTF-8"})
    public @ResponseBody
    BaseRespBean loginPhoneCode(HttpServletRequest request, HttpSession httpSession,
                                @Valid @RequestBody PhoneReqBean reqBean, BindingResult bindingResult) throws IOException {
        BaseRespBean respBean = new BaseRespBean();
        if (bindingResult.hasErrors()) {
            log.warn("bindingResult has error");
            respBean.setResult(ErrorCodeEnum.PARAM_VALID_ERROR);
        ...
        try {
            ...
            if (!(boolean) ((Map) ((Map) mapResp.get("alibaba_aliqin_fc_sms_num_send_response")).get("result"))
                    .get("success")) {
     ...
     * @throws IOException
     */
    @RequestMapping(value = "/login/signPhoneCode", method = RequestMethod.POST, consumes = {
            "application/json"}, produces = {"application/json;charset=UTF-8"})
    public @ResponseBody
    BaseRespBean signPhoneCode(HttpServletRequest request, HttpSession httpSession,
                               @Valid @RequestBody PhoneReqBean reqBean, BindingResult bindingResult) throws IOException {
        BaseRespBean respBean = new BaseRespBean();
        if (bindingResult.hasErrors()) {
            log.warn("bindingResult has error");
     ...
     * @throws IOException
     */
    @RequestMapping(value = "/login/signPhoneCheck", method = RequestMethod.POST, consumes = {
            "application/json"}, produces = {"application/json;charset=UTF-8"})
    public @ResponseBody
    BaseRespBean signPhoneCheck(HttpServletRequest request, HttpSession httpSession,
                                @Valid @RequestBody LoginPhoneReqBean reqBean, BindingResult bindingResult) throws IOException {
        BaseRespBean respBean = new BaseRespBean();
        if (bindingResult.hasErrors()) {
            log.warn("bindingResult has error");
        ...
        if (signPhoneCodeExpireTime == null
                || (Calendar.getInstance().getTimeInMillis() - signPhoneCodeExpireTime.getTime() > 60 * 1000)) {
            respBean.setResult(ErrorCodeEnum.LOGIN_CODE_EXPIRE);
            return respBean;
        } else {
    ...
    @RequestMapping(value = "/login/logout", method = RequestMethod.POST, consumes = {
            "application/json"}, produces = {"application/json;charset=UTF-8"})
    public @ResponseBody
    BaseRespBean logout(HttpSession httpSession,
                        @Valid @RequestBody UserRoleReqBean userRoleReqBean) throws IOException {
    ...
    @RequestMapping(value = "/login/logDetail", method = RequestMethod.POST, consumes = {
            "application/json"}, produces = {"application/json;charset=UTF-8"})
    public @ResponseBody
    BaseRespBean logDetail(@RequestBody BaseReqBean reqBean) throws IOException {
        LogRespBean respBean = new LogRespBean();
        respBean.setLogLevel(LogManager.getRootLogger().getLevel().toString());
        return respBean;
    }

    @RequestMapping(value = "/login/setLog", method = RequestMethod.POST, consumes = {
            "application/json"}, produces = {"application/json;charset=UTF-8"})
    public @ResponseBody
    BaseRespBean setLog(@Valid @RequestBody LogReqBean reqBean, BindingResult bindingResult)
            throws IOException {
        if (bindingResult.hasErrors()) {
            BaseRespBean respBean = new BaseRespBean();
            log.warn("bindingResult has error");
