...
public class UserOperServiceImpl implements IUserOperService {
	...
	public List<Map<String,Object>> getFXJLList() {
		...
		param.put("roleId", "ROLE000014");
		return userDao.userListByRoleId(param);
	}
	...
	public BaseRespBean userLogin(Map<String, Object> paramMap) {
		...
		// 服务器时间取得
		respBean.setServerDate((new SimpleDateFormat("yyyy-MM-dd")).format(new Date()));
	...
	@Override
	@Transactional(readOnly = true)
	public Map<String, Object> userWeChat(String openId) {
		Map<String,Object> param =  new HashMap<>();
		param.put("openId",openId);
		Map<String,Object> resultMap = userDao.userWeChat(param);
		if (resultMap != null){
			List<SysRole> roleList = userDao.selectUserRoleList((String) resultMap.get("userId"));
	...
	@Override
	public BaseRespBean userBind(Map<String, Object> paramMap,String openId) {
		BaseRespBean respBean = new BaseRespBean();
		Map<String, Object> map = userWeChat(openId);
		//验证openId是否绑定
		if (map != null) {
			respBean.setResult(ErrorCodeEnum.USER_BIND_ERROR);
			return respBean;
		}
		Map<String, Object> resultMap = userDao.userLogin(paramMap);
		// 根据用户名密码没有查询到用户
		if (resultMap == null) {
			...
			UserLoginRespBean loginRespBean = new UserLoginRespBean();
			log.info("微信绑定用户{}",loginRespBean.getUsername());
			BeanUtil.mapToBean(resultMap, loginRespBean);
			SysUser user = new SysUser();
			user.setUserId(loginRespBean.getUserId());
	...
	public BaseRespBean updateUserRoleStatus(String userId, String roleId) {
		...
		int result = sysUserRoleMapper.updateByExampleSelective(sysUserRole2, example2);
		if (result < 1) {
	...
	public BaseRespBean changeRole(Map<String, Object> paramMap) {
		...
		String roleId = respBean.getRoleId();
		// 根据角色Id，查询菜单列表
		List<Map<String, Object>> list = menuDao.menuListByRoleId(roleId);
		respBean.setMenuList(ScfBizUtil.menuListToTree(list));

		// 查询商代处
		SysUserRoleExample example = new SysUserRoleExample();
		Criteria criteria = example.createCriteria();
		criteria.andUserIdEqualTo(respBean.getUserId());
		criteria.andRoleIdEqualTo(roleId);
		List<SysUserRole> sysUserRoles = sysUserRoleMapper.selectByExample(example);
		respBean.setRepresentId(sysUserRoles.get(0).getRepresentId());
		return respBean;
	}
	...
	@Override
	@Transactional(readOnly = true)
	public BaseRespBean userListInfo(UserSearchPageReqBean searchPage) {

		Map<String, Object> paramMap = BeanUtil.beanToMap(searchPage);
		// System.out.println("==========================================");
		// System.out.println(searchPage.getUsername());
		// 是否分页，0：否，1：是
		if (searchPage.getIsPage() == 1) {
			PageInfoBean page = new PageInfoBean();
			page.setPageNumber(searchPage.getPageNumber());
			...
			List<Map<String, Object>> resultList = userDao.userListInfo(paramMap, page);
			...
			List<Map<String, Object>> resultList = userDao.userListInfo(paramMap);
			ListRespBean respBean = new ListRespBean();
			respBean.setDataList(resultList);
			return respBean;
		}
		...
		if (hasUser < 1) {
			if (hasmobilephone < 1) {
				...
				for (int i = 0; i < roleIds.length; i++) {
					...
					if ("ROLE000019".equals(roleIds[i])) {
						sysUserRole.setRepresentId(addUserBean.getRepresentId());
					} else {
						sysUserRole.setRepresentId("");
					}
		...
		if (hasUser > 0 && username != null) {
			...
			if (hasmobilephone > 0 && mobilephone != null) {
				...
				for (int i = 0; i < roleIds.length; i++) {
					sysUserRole.setStatus((byte) 0);
					if ("ROLE000019".equals(roleIds[i])) {
						sysUserRole.setRepresentId(userUpdataBean.getRepresentId());
					} else {
						sysUserRole.setRepresentId("");
					}
					if (i == 0) {
	...
	public BaseRespBean updatePwd(UserModPwdReqBean usrModPwdBean) {
		...
		SysUser result = sysUserMapper.selectByPrimaryKey(usrModPwdBean.getUserId());
	...
	public BaseRespBean resetPwd(UserModPwdReqBean usrModPwdBean) {
		...
		SysUser user = sysUserMapper.selectByPrimaryKey(usrModPwdBean.getUserId());
