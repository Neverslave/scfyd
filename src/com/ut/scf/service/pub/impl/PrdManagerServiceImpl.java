...
public class PrdManagerServiceImpl implements IPrdManagerService {
	...
	private static final Logger log = LoggerFactory.getLogger(PrdManagerServiceImpl.class);
	...
	public BaseRespBean getProductList(Map<String, Object> paramMap, PageInfoBean page) {
		...
		if ("0".equals(paramMap.get("isPage").toString())) {
			list = prdInfoDao.getProductIfnoPageList(paramMap);
		} else {
			list = prdInfoDao.getProductIfnoPageList(paramMap, page);
	...
	@Override
	@Transactional(readOnly = true)
	public BaseRespBean getBusinessList(Map<String, Object> paramMap, PageInfoBean page) {
		List<Map<String, Object>> list = null;
		PageRespBean respBean = new PageRespBean();
	
		
			list = businessInfoDao.getBusinessIfnoPageList(paramMap, page);
	...
	public BaseRespBean insertProduct(Map<String, Object> paramMap) {
		...
		record.setBusinessType(Integer.parseInt((String) paramMap.get("businessType")));
		record.setProductType(0);
	...
	public BaseRespBean insertBusiness(Map<String, Object> paramMap) {
		...
		if (businessInfoDao.selectCountById((String) paramMap.get("typeName")) > 0) {
			respBean.setResult(ErrorCodeEnum.BUSINESS_NAME_EXIST);
			respBean.setResultNote("业务类型名称已存在");
			return respBean;
		}
		int maxidint=1;
		String maxid=businessInfoDao.selectMaxId();
		if(maxid!=null)
		{
			maxidint=Integer.parseInt(maxid)+1;
		}
		paramMap.put("typeId", maxidint);
		int insertNum = businessInfoDao.insertBusiness(paramMap);
		log.debug("insert product num {}", insertNum);
		if (insertNum <= 0) {
			respBean.setResult(ErrorCodeEnum.ADD_FAILED);
			return respBean;
	...
	private boolean checkProduct(String productId) {
		...
		com.ut.scf.pojo.auto.DykRateExample.Criteria criteria = example.createCriteria();
		criteria.andProductIdEqualTo(productId);
		List<DykRate> rates = dykRateMapper.selectByExample(example);
	...
	public BaseRespBean deleteProduct(String productId) {
		...
		if (!checkProduct(productId)) {
			respBean.setResult(ErrorCodeEnum.PRODUCT_DELETE_FAIL);
			return respBean;
		}
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public BaseRespBean deleteBusiness(String productId) {
		BaseRespBean respBean = new BaseRespBean();
		/*if (!checkProduct(productId)) {
			respBean.setResult(ErrorCodeEnum.PRODUCT_DELETE_FAIL);
			return respBean;
	 ...
	 * 修改 business
	 * 
	 * @param BusinessTypeReqBean
	 */
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public BaseRespBean updateBusiness(BusinessTypeReqBean reqBean) {
		BaseRespBean respBean = new BaseRespBean();
		Map<String, Object> paramMap = BeanUtil.beanToMap(reqBean);
		if (businessInfoDao.selectCountByIdExcepid((String) paramMap.get("typeId"),(String) paramMap.get("typeName")) > 0) {
			respBean.setResult(ErrorCodeEnum.BUSINESS_NAME_EXIST);
			respBean.setResultNote("业务类型名称已存在");
			return respBean;
		}
		int updateNum = businessInfoDao.updateBusiness(paramMap);
		log.debug("update business num {}", updateNum);
		if (updateNum <= 0) {
			respBean.setResult(ErrorCodeEnum.UPDATE_FAILED);
			return respBean;
		}
