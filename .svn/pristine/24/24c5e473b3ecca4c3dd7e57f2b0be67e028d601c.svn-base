$(document).ready(function() {
	$("#riskManager").hide();
   initFileTable("projectFiles", "material_type_report");
...
function loadTaskData() {
    ...
    data.taskId = taskId;
    var options = {
        url: '../../tradfact/recommeet/bpm/getTaskDataByTaskId',
        data: JSON.stringify(data),
        callBackFun: function(data) {
            if (data.result == 0) {
                data = eval("(" + data.str + ")");
                CloudUtils.setForm(data, 'recomMeetForm');
                CloudUtils.setForm(data, 'recomInfoForm');
                tables = $(".table");
...
function loadformDate(projectId){
	  ...
	  var options = {
		        ...
		        callBackFun: function(data) {
		            if (data.result == 0) {
		                CloudUtils.setForm(data.dataList[0], 'recomInfoForm');
		            } else {
		                return false;
		            }
		        },
...
function agree() {
    var options = {
        url: '../../tradfact/recommeet/bpm/auditProcess',
...
function reapply() {
    var options = {
        url: '../../tradfact/recommeet/bpm/reApply',
        data: JSON.stringify(getData()),
        callBackFun: function(data) {
            bootbox.alert(data.resultNote, function() {
                window.location.href = '../../project/agencyTask/agencyTask.html';
...
function readOrWrite(){
	if(taskDefKey == "usertask1"){
		$("#upwardsInfo").attr("readOnly",false);
	}
}
