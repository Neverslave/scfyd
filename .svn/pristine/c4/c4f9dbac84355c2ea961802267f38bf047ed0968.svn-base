...
public class FileUploadController extends BaseJsonController {
	...
	public void uploadFile(
		...
		double fileSize = Math.ceil((double) (file.getSize() / 1024) / 1024);

		if (fileSize > 50) {
			resp.setResult(-1);
			resp.setResultNote("上传文件不得超过50M");
		} else {
			File targetFile = new File(path, fileName);
			if (!targetFile.exists()) {
				targetFile.mkdirs();
			...
			String[] tempStrs = fileName.split("\\.");
			resp.setFileSize(file.getSize() / 1024);
			resp.setFileName(uploadName);
			resp.setFileType(tempStrs[1]);
			resp.setFileUrl(request.getContextPath() + "/" + uploadFilePath
					+ fileName);
			resp.setFilePath(request.getContextPath() + "/" + uploadFilePath
					+ fileName);
		}
	...
	// public @ResponseBody BaseRespBean base64Upload(HttpServletRequest
	// request,
