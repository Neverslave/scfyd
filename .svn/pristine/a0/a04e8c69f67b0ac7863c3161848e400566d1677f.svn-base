				   ...
				   return $.number(value,2);
		 	       ...
		 	       valign: 'middle',
					 formatter:function(value,row,index){
						 if(row.payAbleGuarantee<row.payActGuarantee){
							 return $.number(0,2);
						 }
					   return $.number(value,2);
					  }
...
function showChart(data){
 ...
 legend: {
     orient: 'vertical',
     x: 'left',
     data:['总计冲抵保证金金额','总计实缴保证金金额','总计未缴保证金金额','总计剩余保证金金额']
 },
 series: [
     {
         name:'访问来源',
     ...
     },
     {
         ...
         data:[
               {value:offsetGuaranteeTotal, name:'总计冲抵保证金金额'}
