...
function InitTable() {
    ...
    $("#handleTable").bootstrapTable({
        ...
        columns: [{
         ...
         /*       var epress=''
                if(row.procdefName == 'dyk还款申请'){
                	epress = '<a class = "glyphicon glyphicon-envelope envelope" style="color:#278bdd;padding:0px 5px;" title="快递信息" href="javascript:void(0)"></a>';
                }*/
    		...
    		callBackFun : function(data) {
    			if (data.result == 0) {
    					var list=data.dataList
    					 taskId=data.dataList[list.length-1].taskId;
    			} else {
    				return false;
    			}
    		},
