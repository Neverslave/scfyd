...
public class UploadFileRelaController extends BaseJsonController{
	...
	@RequestMapping(value = "/informFileList", method = RequestMethod.POST, consumes = { "application/json" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody BaseRespBean getInformFileList(HttpSession httpSession,
			@RequestBody InformFileListReqBean reqBean, BindingResult bindingResult) throws IOException{
		BaseRespBean respBean = new BaseRespBean();
		if (bindingResult.hasErrors()) {
			log.warn("bindingResult has error");
			respBean.setResult(ErrorCodeEnum.PARAM_VALID_ERROR);
		...
		page.setPageNumber(reqBean.getPageNumber());
		page.setPageSize(reqBean.getPageSize());
		respBean = uploadFileRelaService.getInformFileList(paramMap,page);
		return respBean;
	}
}
