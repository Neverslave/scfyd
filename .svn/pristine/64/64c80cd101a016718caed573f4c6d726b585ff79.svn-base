...
public class ActivitiServiceImpl implements IActivitiService {
	...
	private void getAgencyTaskList(List<Map<String, Object>> list){
		for (Map<String, Object> taskMap : list) {
			...
			String originAssigneeGs="";
			String preAssigneeGS="";
			if (AllTaskList.size() >= 2) {
				...
				if(preUsername.indexOf("(")>0){
					preAssigneeGS=preUsername;
				}else{
				CorpInfo corpInfo = corpInfoMapper.getByUsername(preUsername);
				if(corpInfo!=null){
					preAssigneeGS=corpInfo.getCorpName();
				}else{
					preAssigneeGS=preUsername;
				}
				}
				taskMap.put("preAssignee", preTask.getAssignee());
				taskMap.put("preAssigneeGs", preAssigneeGS);
				//通过username获取获取经销商名称
				String username=(String) taskService.getVariable((String) taskMap.get("taskId"), "launchUser");
				
				if(username.indexOf("(")>0){
					originAssigneeGs=username;
				}else{
						CorpInfo corpInfo1 = corpInfoMapper.getByUsername(username);
						if(corpInfo1!=null){
							originAssigneeGs=corpInfo1.getCorpName();
						}else{
							originAssigneeGs=username;
						} 
				}
					
				taskMap.put("originAssigneeGs", originAssigneeGs);// 获取发起者公司名称
				taskMap.put("originAssignee", username);// 获取发起者
	...
	public BaseRespBean getHandleTaskList(Map<String, Object> map,
		...
		for (Map<String, Object> map2 : list) {
			...
			if(assigneeName.indexOf("(")>0){
				assigneeGS=assigneeName;
			}else{
				CorpInfo corpInfo = corpInfoMapper.getByUsername(assigneeName);
				if(corpInfo!=null){
					assigneeGS=corpInfo.getCorpName();
				}else{
					assigneeGS=assigneeName;
				}
			}
			map2.put("assignee", assigneeName);
			map2.put("assigneeGs", assigneeGS);
