...
! function($) {
    ...
    BootstrapTable.prototype.initTable = function() {
        ...
        $.each(this.options.columns, function(i, column) {
            that.options.columns[i] = $.extend({}, BootstrapTable.COLUMN_DEFAULTS, { field: i }, column); // when field is undefined, use index instead
        });
                ...
                if (that.options.columns[j].checkbox || that.options.columns[j].radio) {
                    ...
                    if (typeof value === 'string' && value.indexOf("<a")== 0) {
                        ...
                        text = that.options.cardView ? ['<div class="card-view">',
                            ...
                            sprintf('<span class="value">%s</span>', value),
                            '</div>'
                        ].join('') : [sprintf('<td title = "%s" %s %s %s>', value, id_, class_, style),
                            value,
                            '</td>'
                        ].join('');
