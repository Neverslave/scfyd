	...
	var options = {
		...
		callBackFun : function(data) {
			if (data.result == 0) {
				...
				if(jsonData.payAmitInfo!=null&&jsonData.payAmitInfo!="")
					{
					$("#payAmitListTableDetail").bootstrapTable('append', JSON.parse(jsonData.payAmitInfo));
					var repayAcuAmit=0;
...
function numFormat(){
	...
	$("#repayDetailForm #repayAutAmt").number(true, 2);
}

function initAmitTable() { 
	$('#payAmitListTableDetail')
