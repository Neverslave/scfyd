...
(function ($){
	/*document.write('<script language="javascript" src="js/perfectLoad.js"></script>');*/
	jQuery.bootstrapLoading = {
	    start: function (options) {
	        ...
	        //获取页面宽高
	        var _PageHeight = document.documentElement.scrollHeight,
	        _PageWidth = document.documentElement.scrollWidth;
	        

	        //在页面未加载完毕之前显示的loading Html自定义内容
	        var _LoadingHtml = '<div id="loadingPage" style="left:0;top:0;position: absolute;width:100%;z-index: 10001;filter:alpha(opacity=60);-moz-opacity: 0.5;height:' + _PageHeight + 'px;background:' + options.backgroundColor + ';opacity:' + options.opacity + ';filter:alpha(opacity=' + options.opacity * 100 + ');"><div id="loadingTips" style="width:310px;cursor1: wait;margin: auto;  position: absolute;  top: 0; left: 0; bottom: 0; right: 0;border-color:' + options.borderColor + ';border-style:' + options.borderStyle + ';border-width:' + options.borderWidth + 'px; height:80px; line-height:80px; padding-left:80px; padding-right: 5px;border-radius:10px;  background: ' + options.backgroundColor + ' url(/scfyd/images/loading-082.gif) no-repeat 5px center; color:' + options.TipsColor + ';font-size:20px;">' + options.loadingTips + '</div></div>';
