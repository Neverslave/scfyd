    ...
    var options = {
        ...
        callBackFun: function(data) {
            $.each(data.dataList, function(index, units) {
                $("#areaId").append("<option value=" + units.areaId + ">" + units.areaName + "</option>");
            });
        }
    };
...
function changeArea() {
    ...
    var options = {
        ...
        callBackFun: function(data) {
            $("#representId").html("");
            $.each(data.dataList, function(index, units) {
                $("#representId").append("<option value=" + units.representId + ">" + units.representName + "</option>");
            });
        }
    };
    CloudUtils.ajax(options);
...
function changeRole() {
    ...
    var index = $.inArray("ROLE000019", roleIds);
    if (index != -1) {
...
function inputPassword() {
    $('#mobilephone').bind('input propertychange', function() {
        ...
        if (mobileNum.length > 6) {
            password = mobileNum.substr(mobileNum.length - 6);
        } else {
            password = mobileNum;
        }
        ...
        if (roleId == row.userId) {
            ...
            bootbox.confirm("确定删除此条记录?", function(result) {
                if (result) {
                    var options = {
                        url: '../../user/delete',
                        data: '{"userId":"' + row.userId + '","userName":"' + row.username + '"}',
                        callBackFun: function(data) {
                            if (data.result == 0) {
                                searchFun();
                            } else {
...
function addFun() {
    $("#searchFire").attr("disabled", false);
    $("#addForm")[0].reset();
    $("#addForm").bootstrapValidator('resetForm', true);
    $("#deptInfo").hide();
    ...
    $("#sysType option:first").prop("selected", true);
...
function modFun(row) {
    ...
    $('#roleId').val(roleIdList).trigger('change');
                    ...
                    if (data.result == 0) {
                        ...
                        bootbox.alert(data.resultNote).on('hide.bs.modal', function() {
                            ...
                            for (var i = 0; i < roleId.length; i++) {
                                $("#roleId option[value=" + roleId[i] + "]").attr("selected", "selected");
                            }
                        });
                    }
                },
...
function changeType(obj) {
    $("#roleId").select2("val", ""); //更改系统类型时，清除已选择的用户角色
    $("#deptInfo input").val("");
    //$("#deptId").html("");
    ...
    $("#corpName").val("");
    $("#deptId").val("");
    $('#addForm').data('bootstrapValidator').updateStatus('roleId', 'NOT_VALIDATED').validateField('roleId');
    $('#addForm').data('bootstrapValidator').updateStatus('corpName', 'NOT_VALIDATED').validateField('corpName');
}
...
function checkCorpId() {
    ...
    $("#addForm")
        ...
        .updateStatus("corpName", "NOT_VALIDATED", null)
        .validateField("corpName");
